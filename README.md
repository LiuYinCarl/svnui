...
| **暂存/提交** | `space` 暂存（加入提交集），暂存未版本化文件会自动执行 `svn add`；`svn commit` 只提交暂存的文件（未暂存时提交全部变更）。输入框基于 `tui-textarea-2`：**中文/emoji 等宽字符时光标严格按单元格对齐**，退格按字符安全删除 |
| **日志视图** | `svn log -v` 修订列表 + 变更路径与提交信息详情；可查看单个修订的 diff |
