...
- **ci.yml** — push / PR 时运行：fmt、clippy（零警告门禁）、Linux/macOS 全量测试、覆盖率门禁（≥ 80%）、三平台 release 构建、压测（浅克隆一个轮换的开源仓库 redis / clap / slugify，用 git2svn 转成 SVN 后无头驱动 App 跑 60 轮随机操作；日志记录源仓库 commit 以便复现）。
